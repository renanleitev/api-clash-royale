...
const findCardImage = async (card) => {
  ...
  const battle = await Battle.findOne({
    $or: [
      { "player1Deck.name": card },
      { "player2Deck.name": card }
    ]
  });
...
router.get('/defeats-by-card-combo', async (req, res) => {
  ...
  try {
    ...
    const cardImages = await Promise.all(comboArray.map(async card => await findCardImage(card)));
...
router.get('/combos-wins-percentage', async (req, res) => {
  ...
  if (!deckQuantity || !winPercentage || !startTime || !endTime) {
    return res.status(400).json({ message: 'Parâmetros deckQuantity, winPercentage, startTime e endTime são necessários!' });
  }
  ...
  try {
    ...
    const battlesFiltered = battles.filter(battle => 
    ...
    const result = Object.keys(deckStats).map(deckKey => {
